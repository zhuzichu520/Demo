...
public class TeamNotificationHelper {
    ...
    private static String getTeamMemberDisplayName(String account) {
        return ToolTeam.INSTANCE.getTeamMemberDisplayNameYou(teamId.get(), account);
    }
    ...
    private static String buildInviteMemberNotification(MemberChangeAttachment a, String fromAccount) {
        ...
        Team team = ToolTeam.INSTANCE.getTeamById(teamId.get());
    ...
    private static String buildUpdateTeamNotification(String tid, String account, UpdateTeamAttachment a) {
        ...
        for (Map.Entry<TeamFieldEnum, Object> field : a.getUpdatedFields().entrySet()) {
            if (field.getKey() == TeamFieldEnum.Name) {
                ...
                sb.append(ToolTeam.INSTANCE.getTeamMemberDisplayNameYou(tid, account) + " 修改了群公告");
                ...
                if (type == VerifyTypeEnum.Free) {
                    sb.append(authen + AppGlobal.INSTANCE.getContext().getString(R.string.team_allow_anyone_join));
                } else if (type == VerifyTypeEnum.Apply) {
                    sb.append(authen + AppGlobal.INSTANCE.getContext().getString(R.string.team_need_authentication));
                } else {
                    sb.append(authen + AppGlobal.INSTANCE.getContext().getString(R.string.team_not_allow_anyone_join));
                }
